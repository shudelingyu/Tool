...
def send_movement_command(mode):
    ...
    try:
        ...
        try:
            ...
            distance_val = math.radians(distance_val)
        except ValueError:
        ...
        try:
            ...
            messagebox.showwarning("警告", "循环次数必须为正整数")
            return
        
        # 构建指令（包含循环次数）
        command = f"MoveTest --mode={mode} --distance={distance_val} --cycles={cycles}"
        # 构建指令
        # command = f"MoveTest --mode={mode} --distance={distance_val}"
